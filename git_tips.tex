...
\begin{enumerate}
	...
	\item In order to merge, first make sure that Your local main branch is up-to-date with the remote one. In order to do that, \textit{switch} to main branch and \textit{pull} from the remote.
	\item Then, \textit{merge} Your branch into main. Solve any conflicts that may arise.
...
Suppose You started working on Your branch and already made some commits. In the meantime, however, one of Your teammates merged their work into main and now Your branch is outdated. In such a case, the commit graph may look (roughly) like in Figure \ref{fig:before_rebase}. You may want to update Your branch and keep working on it, without having to merge. There is an excellent tool that can help You with that - it's called \textit{Git Rebase}. After rebasing, Your commits will appear as if they were made after the teammate's merge. The results would be similar to those in Figure \ref{fig:after_rebase}.\newline
...
\draw[->] (main.west) .. controls +(left:4mm) and +(down:4mm) .. (branch.south);
\draw[->] (branch.north) -- (branch2.south);
\end{tikzpicture}
\captionof{figure}{Before rebase.}
\label{fig:before_rebase}
\end{minipage}
\hspace{0.1\textwidth}
\begin{minipage}[b]{0.45\textwidth}
\begin{tikzpicture}[
...
\node[mainnode](main2)[above=of main]{};
\node[mainnode](main3)[above=of main2, label=right:\textcolor{OliveGreen}{$\mathtt{main}$}]{};
\node[branchnode](branch)[above left=of main3]{};
\node[branchnode](branch2)[above=of branch, label=left:\textcolor{BrickRed}{$\mathtt{branch}$}]{};
\draw[->] (main.north) -- (main2.south);
\draw[->] (main2.north) -- (main3.south);
\draw[->] (main3.west) .. controls +(left:4mm) and +(down:4mm) .. (branch.south);
\draw[->] (branch.north) -- (branch2.south);
\end{tikzpicture}
\captionof{figure}{After rebase.}
\label{fig:after_rebase}
\end{minipage}
\newline
...
\draw[->] (main.west) .. controls +(left:4mm) and +(down:4mm) .. (branch.south);
\draw[->] (branch.north) -- (branch2.south);
\draw[->] (branch2.north) .. controls +(up:4mm) and +(left:4mm) .. (main4.west);
\end{tikzpicture}
\caption{After merge.}
